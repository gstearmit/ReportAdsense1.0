...
class Email implements ServiceLocatorAwareInterface 
{
    ...
    private function logWrite($message) {
        ...
        $domainaname = $_SERVER["SERVER_NAME"];
        $check = "www";
         
        if(strpos($domainaname,$check) !== false) { 
        	$phuc = 0;
        	$txt = explode(".",$domainaname); 
        	if(count($txt)>2) {
        		$txtfix = $txtfix.$txt[1]."/../";
        	}  
        }
        ...
        if(strpos($domainaname,$check) === false)
        { 
        	if ( $domainaname == '127.0.0.1' || $domainaname == '::1' || $domainaname == 'localhost' ){
        		$txtfix= $txtfix.'../';
        		$phuc = 1;
        	}else {
	        	$txt=explode(".",$domainaname);  
        ...
        //stream writer         
        $writerStream = new \Zend\Log\Writer\Stream( $_SERVER['DOCUMENT_ROOT'].$txtfix. 'data/logs/' . date('Ymd') . '-log.txt');
